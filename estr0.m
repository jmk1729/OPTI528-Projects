function [windSpeed, Vrms, r0] = estr0(month,rmsFlag,zenAngle,altitude)
...
if rmsFlag == true
    ...
    Cn2 = @(h) (0.00594*((Vrms/27).^2).*(((10^-5).*h).^10).*exp(-h./1000) ...
        +(2.7*10^-16)*exp(-h./1500) + A.*exp(-h./100)).*(h/HighAlt).^(5/3); % HVModel
...
%     title('C_n^2 vs Altitude for Tucson');
    ...
    r0 = (0.423*k^2*secd(zenAngle)*integral(Cn2,LowAlt,HighAlt)).^(-3/5); % Dyson
else
    r0 = 'Not Computed';
end
end